...
export class ExchangeRatePlugin extends plugin {
  ...
  generateRules() {
    ...
    this.rule.push({
      reg: new RegExp(`.*?(\\d*\\.?\\d*)?\\s*(${currencyKeys})转(${currencyKeys}).*`),
      fnc: "convertCurrency",
    });
  }

  async getExchangeRate(e) {
    const match = e.msg.match(new RegExp("(" + Object.keys(reverseCurrencyMap).join('|') + ")汇率"));
    if (match) {
  ...
  async convertCurrency(e) {
    const match = e.msg.match(new RegExp("(\\d*\\.?\\d*)?\\s*(" + Object.keys(reverseCurrencyMap).join('|') + ")转(" + Object.keys(reverseCurrencyMap).join('|') + ")"));
    if (match) {
      ...
      try {
        ...
        if (data && data.exchange_rate) {
          ...
          e.reply(`${amount} ${data.currency1_name} (${data.currency1}) = ${convertedAmount} ${data.currency2_name} (${data.currency2})`);
        } else {
          e.reply("未能获取汇率，请稍后再试。");
        }
      } catch (error) {
