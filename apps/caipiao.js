...
export class Example extends plugin {
  ...
  async buyLottery(e) {
    ...
    if (data[data.length - 1] && turns == data[data.length - 1].turns) {
      //买过彩票则输出彩票信息
      ...
      await e.reply(
        this.formatLottery(
          ...
          nickname,
          userId,
          userLottery.lotteryNum,
          userLottery.groupId,
          userLottery.groupName,
          userLottery.time,
          groupId == userLottery.groupId
        ),
        true
      );
      return;
    ...
    } else if (e.raw_message.length != 7) {
      //购买彩票号码大于3位数字
      await e.reply("彩票号码为2位数，请重新购买，此次不做记录", 1);
    } else {
      //指定
      lotteryNum = e.raw_message.slice(5).trim();
      if (lotteryNum < 10) {
        await e.reply(`号码应在10-99之间`, true);
        return;
      }
      let temp = info[turns - 1].used;
      if (!temp || !temp.find((item) => item.lotteryNum == lotteryNum)) {
        //或运算前为used数组为空，直接添加；或运算后为在temp中寻找相同的lotteryNum，买找到返回0，则可以添加
        userLottery = {
          turns: turns,
          lotteryNum: lotteryNum,
          groupId: groupId,
        ...
        await e.reply(
          `该号码(${lotteryNum})已经被其他人购买,请换一个号码`,
          true
        );
        return;
      }
  ...
  async sellLottery(e) {
    ...
    if (!fs.existsSync(`./data/lottery/user/${userId}.json`)) {
      //未购买彩票
    ...
    if (userLottery.turns != info.length) {
      //最后的轮次与当前轮次不同，无法卖出
  ...
  async draw(e) {
    ...
    let person = thisTurn.used.find(
      (item) => item.lotteryNum == thisTurn.award
    );
    ...
    if (!info || !thisTurn) {
      await e.reply(
        "错误!可能是从未运行过该插件导致文件缺失，请尝试执行以下指令“#购买彩票”",
        true
      );
      return;
    }
    ...
    if (!person) {
      //person数组不存在，无人中奖
      await e.reply(
        `很遗憾，本轮无人买中，本次中奖号码为${info[info.length - 1].award}`
      );
      historyContent = {
        //记录历史
      ...
      await e.reply(
        `恭喜${person.nickname}(${person.userId})中奖!中奖号码为${person.lotteryNum}`
      );
  ...
  async lotteryHistory(e) {
    ...
    for (let i = 1; i <= history.length; i++) {
      ...
      msg += `中奖人: ${replyInfo.nickname || "无人中奖"} (${
        replyInfo.userId || "NULL"
      })\n`;
  ...
  async myHistory(e) {
    ...
    for (let i = 1; i <= userHistory.length; i++) {
      if (lotteryInfo.length == userHistory[i - 1].turns) {
        //记录的最后一轮和用户的轮次一样，说明是最后一轮且未开奖，不能告知是否中奖
        replyInfo = {
        ...
        replyInfo = {
          ...
          time: userHistory[i - 1].time,
          isDraw:
            lotteryInfo[userHistory[i - 1].turns - 1].award ==
            userHistory[i - 1].lotteryNum,
        };
      }
      ...
      msg += `是否中奖: ${
        replyInfo.isDraw == "未开奖" ? "未开奖" : replyInfo.isDraw ? "是" : "否"
      }\n`;
      msg += "-------------\n";
    }
  ...
  async myLottery(e) {
    ...
    if (turns == data[data.length - 1].turns) {
      //当前轮次买过彩票则输出彩票信息
      userLottery = data[data.length - 1];
      await e.reply(
        this.formatLottery(
